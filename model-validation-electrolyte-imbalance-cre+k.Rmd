...
newdata_num <- data.frame(cbind(age, severe_cns, bleeding, cr,  k))
...
age_val <- unlist(map(newdata$age, function(x) x * 0.03801523))
severe_cns_val <- unlist(map(newdata$severe_cns,  function(x) x * 1.03407785))
bleeding_val <-  unlist(map(newdata$bleeding, function(x) x * 0.95297837))
cr_val <- unlist(map(newdata$cr, function(x) x * 0.16937162))
k_val <-  unlist(map(newdata$k, function(x) x * 0.85508951))

coef_df <- cbind.data.frame(age_val, severe_cns_val, bleeding_val, cr_val, k_val)
