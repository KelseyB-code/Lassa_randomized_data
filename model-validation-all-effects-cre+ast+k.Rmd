...
newdata_num <- data.frame(cbind(age, severe_cns, bleeding, ast, cr,  k))
...
```{r}
intercept <- -8.124503910
age_val <- unlist(map(newdata$age, function(x) x * 0.040699190))
severe_cns_val <- unlist(map(newdata$severe_cns,  function(x) x * 1.074291571))
bleeding_val <-  unlist(map(newdata$bleeding, function(x) x * 0.916527884))
ast_val <- unlist(map(newdata$ast,  function(x) x * 0.003104113))
cr_val <- unlist(map(newdata$cr, function(x) x * 0.142484771))
k_val <-  unlist(map(newdata$k, function(x) x * 0.875683806))

coef_df <- cbind.data.frame(age_val, severe_cns_val, bleeding_val, ast_val, cr_val, k_val)
