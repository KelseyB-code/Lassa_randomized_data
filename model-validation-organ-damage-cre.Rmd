...
newdata_num <- data.frame(cbind(age, severe_cns, bleeding, cr))
...
```{r}
intercept <- -3.61213906
age_val <- unlist(map(newdata$age, function(x) x * 0.03319482))
severe_cns_val <- unlist(map(newdata$severe_cns,  function(x) x * 1.41547045))
bleeding_val <-  unlist(map(newdata$bleeding, function(x) x * 0.79123974))
cr_val <- unlist(map(newdata$cr, function(x) x * 0.18242345))

coef_df <- cbind.data.frame(age_val, severe_cns_val, bleeding_val, cr_val)
