...
newdata_num <- data.frame(cbind(age, severe_cns, bleeding, ast, cr))
...
age_val <- unlist(map(newdata$age, function(x) x * 0.035277727))
severe_cns_val <- unlist(map(newdata$severe_cns,  function(x) x * 1.429050048))
bleeding_val <-  unlist(map(newdata$bleeding, function(x) x * 0.768763947))
ast_val <- unlist(map(newdata$ast,  function(x) x * 0.003146318))
cr_val <- unlist(map(newdata$cr, function(x) x * 0.174740591))

coef_df <- cbind.data.frame(age_val, severe_cns_val, bleeding_val, ast_val, cr_val)
