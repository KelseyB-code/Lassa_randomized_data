...
newdata_num <- data.frame(cbind(age, severe_cns, bleeding,  k))
...
```{r}
intercept <- -7.81439095
age_val <- unlist(map(newdata$age, function(x) x * 0.04313116))
severe_cns_val <- unlist(map(newdata$severe_cns,  function(x) x * 1.05637802))
bleeding_val <-  unlist(map(newdata$bleeding, function(x) x * 0.99932872))
k_val <-  unlist(map(newdata$k, function(x) x * 1.04451396))

coef_df <- cbind.data.frame(age_val, severe_cns_val, bleeding_val, k_val)
